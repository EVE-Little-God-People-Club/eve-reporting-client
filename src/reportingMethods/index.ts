...
export class ReportingMethodsMediator {
  ...
  private constructor() {
    EventCenter.instance.listen<boolean>("quiet", isQuiet => this.isQuiet = isQuiet)
    EventCenter.instance.listen<string>("warn", title => this.isQuiet && this.emitWarn(title))
    EventCenter.instance.listen<string>("reminder", title => this.isQuiet && this.emitReminder(title))
    EventCenter.instance.listen<undefined>("refresh", () => this.refresh())
  }
  ...
  public refresh() {
    getConfig().then(cfg => {
      ...
      if (cfg.notificationEnabled) {
        this.addReportMethod(new Notification(
      ...
      if (cfg.vibrateEnabled) {
        this.addReportMethod(new Vibrate(
